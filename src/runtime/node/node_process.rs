...
/// process.getActiveResourcesInfo(). Only ref'd, open handles are counted
/// (Node lists handles that keep the loop alive; an `unref()`ed socket or a
/// stopped server is excluded). Per-thread.
#[unsafe(no_mangle)]
pub(crate) extern "C" fn Bun__getActiveResourceCounts(
    ...
    let Some(ar) = crate::jsc_hooks::active_resources() else {
        return;
    };
    // A socket is "active" once `on_open` has marked it (`Flags::IS_ACTIVE`);
    // server-accepted sockets do not arm their own `poll_ref` (the listener's
    // ref keeps the loop alive), so `poll_ref.is_active()` would under-count.
    // Node lists every ref'd TCP handle — accepted sockets are ref'd by
    // default there — so `IS_ACTIVE` is the closest analogue here.
    let mut n_sockets = 0usize;
    for &addr in ar.tcp_sockets.keys() {
        // SAFETY: address registered in `NewSocket::new`, removed in
        // `deinit_and_destroy` before the heap free; both run on this thread.
        let s = unsafe { &*(addr as *const TCPSocket) };
        if s.flags
            .get()
            .contains(crate::socket::SocketFlags::IS_ACTIVE)
        {
            n_sockets += 1;
        }
    }
    for &addr in ar.tls_sockets.keys() {
        // SAFETY: as above for `NewSocket<true>`.
        let s = unsafe { &*(addr as *const TLSSocket) };
        if s.flags
            .get()
            .contains(crate::socket::SocketFlags::IS_ACTIVE)
        {
    ...
    for &addr in ar.listeners.keys() {
        // SAFETY: address registered in `Listener::listen`, removed in
        // `Listener::deinit` before the heap free; both run on this thread.
        let l = unsafe { &*(addr as *const Listener) };
        if l.poll_ref.get().is_active() && !matches!(l.listener.get(), ListenerType::None) {
            n_servers += 1;
        }
    }
    // SAFETY: out-params are valid pointers from the C++ caller.
    unsafe {
        *tcp_sockets = n_sockets;
        *tcp_servers = n_servers;
        *fs_requests = ar.fs_requests.get();
    }
}

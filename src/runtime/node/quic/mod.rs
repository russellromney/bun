//! Native binding for `node:quic` — Bun's equivalent of Node's
...
//! via `$zig("quic.zig", "createNodeQuicBinding")` and immediately registers
//! its callbacks through `setCallbacks()`. Constant names and values mirror
//! Node exactly; state byte offsets are derived from the `#[repr(C)]` state
//! structs that back the handles' `state` buffers so they cannot drift.
//!
//! Skeleton phase: constants, callback registration, and `QuicEndpoint`
//! lifecycle exist; networking (listen/connect, sessions, streams) is not
//! implemented yet.
...
pub(crate) mod callbacks {
    ...
    pub(crate) fn get(global: &JSGlobalObject, name: &str) -> Option<JSValue> {
        let holder = global.bun_vm().as_mut().rare_data().node_quic_callbacks.get()?;
...
/// `$zig("quic.zig", "createNodeQuicBinding")`.
pub(crate) fn create_node_quic_binding(global: &JSGlobalObject) -> JsResult<JSValue> {
    ...
    obj.put(
        global,
        b"setCallbacks",
        JSFunction::create(global, "setCallbacks", __jsc_host_set_callbacks, 1, Default::default()),
    );
    ...
    put_num(obj, global, "CLOSECONTEXT_CLOSE", endpoint::CLOSECONTEXT_CLOSE as u64);
    put_num(obj, global, "CLOSECONTEXT_BIND_FAILURE", endpoint::CLOSECONTEXT_BIND_FAILURE as u64);
    put_num(obj, global, "CLOSECONTEXT_LISTEN_FAILURE", endpoint::CLOSECONTEXT_LISTEN_FAILURE as u64);
    put_num(obj, global, "CLOSECONTEXT_RECEIVE_FAILURE", endpoint::CLOSECONTEXT_RECEIVE_FAILURE as u64);
    put_num(obj, global, "CLOSECONTEXT_SEND_FAILURE", endpoint::CLOSECONTEXT_SEND_FAILURE as u64);
    put_num(obj, global, "CLOSECONTEXT_START_FAILURE", endpoint::CLOSECONTEXT_START_FAILURE as u64);

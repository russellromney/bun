...
JSC_DEFINE_HOST_FUNCTION(jsFunctionNodeHTTPServerSocketIsRequestTimedOut, (JSC::JSGlobalObject * globalObject, JSC::CallFrame* callFrame))
{
    ...
    // re-validation, so clamp before the cast.
    constexpr double kU64Max = 18446744073709551615.0;
    uint64_t headersTimeoutMs = std::isfinite(headersTimeout) && headersTimeout > 0 ? static_cast<uint64_t>(std::min(headersTimeout, kU64Max)) : 0;
    uint64_t requestTimeoutMs = std::isfinite(requestTimeout) && requestTimeout > 0 ? static_cast<uint64_t>(std::min(requestTimeout, kU64Max)) : 0;

...
impl TranspilerJob {
    ...
    pub(crate) fn run(&mut self) {
        ...
        if !matches!(parse_result.already_bundled, AlreadyBundled::None) {
            // The bytecode lives in `AlreadyBundled::Bytecode(Box<[u8]>)`, which
            // would drop when `parse_result` drops on return. This job is then
            // dispatched to the JS thread (via the `dispatch_to_main_thread`
            // scopeguard) after that drop, so a borrowed pointer would hand C++
            // freed memory. Move the variant out and `heap::into_raw` so
            // ownership transfers to C++ — mirrors the synchronous path in
            // `jsc_hooks.rs`.

...
impl<'a> Context<'a> {
    ...
    pub fn new(config: Config<'a>, pwd: &'a [u8], salt: &'a [u8]) -> Result<Context<'a>> {
        ...
        // Bun patch: accept any `mem_cost >= 1` so hashes produced by Bun's
        // earlier Zig-based implementation (which allowed `memoryCost >= 1`)
        // remain verifiable. `memory_blocks` is clamped to `8 * lanes` below,
        // and the raw `mem_cost` is what enters H0, matching RFC 9106 and the
        // Zig stdlib behaviour. See src/deps/rust-argon2/README.md.
...
mod tests {
    ...
    #[test]
    fn new_with_zero_mem_cost_returns_correct_error() {
        // Bun patch: only `mem_cost == 0` is rejected; values in 1..8*lanes
        // are clamped up instead of rejected so legacy hashes stay verifiable.
        let config = Config {
            mem_cost: 0,
            ..Default::default()
        };
    ...
    #[test]
    fn new_with_less_than_8_x_lanes_mem_cost_is_clamped() {
        // Bun patch: `mem_cost < 8 * lanes` is accepted and `memory_blocks`
        // is clamped to `2 * SYNC_POINTS * lanes`.
        let config = Config {
            lanes: 4,
            mem_cost: 31,
            ..Default::default()

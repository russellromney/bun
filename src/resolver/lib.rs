...
pub mod dir_entry_accessor {
    ...
    pub(crate) struct DirEntryNameWrapper {
        ...
        // (`base_: StringOrTinyString`). The bytes live inline in the `Entry`
        // (tiny) or in the 'static `FilenameStore` (long); the `Entry` itself
        // is owned by the EntryStore BSSList singleton, so they stay live while
        // the parent `DirEntry` is. Stored as [`bun_ptr::RawSlice`] (not
        // `&'static [u8]`) per PORTING.md §Forbidden — for a tiny string the
        // bytes live inside the `Entry`, not in a 'static arena, so minting a
        // `'static` borrow via `from_raw_parts` would be a lifetime lie.
        // `RawSlice` encapsulates the outlives-holder invariant so `slice()`
        // is safe.
    ...
    impl AccessorDirIter for DirEntryDirIter {
        ...
        fn next(&mut self) -> Maybe<Option<DirEntryIterResult>> {
            if let Some(value) = &mut self.value {
                // The map key is lowercased (see `DirEntry::add_entry`, which
                // stores each entry under `base_lowercase()` for case-insensitive
                // lookups). The glob walker needs the original-case basename —
                // handing it the lowercased key makes it `openat()` a path that
                // does not exist on a case-sensitive filesystem, surfacing ENOENT.
                ...
                // BACKREF: wrap the entry's original-case basename in a `RawSlice`.
                // `base_` is a `StringOrTinyString`: its bytes live inline in the
                // `Entry` (tiny) or in the 'static `FilenameStore` (long). The
                // `Entry` is owned by the EntryStore BSSList singleton, so the
                // bytes outlive this wrapper just as the HashMap key would, and
                // `name_slice()` re-narrows the lifetime so it never escapes.

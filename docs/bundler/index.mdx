...
Works for both CommonJS and ESM output, with or without `compile: true`. Without `compile`, a `.jsc` bytecode file is generated alongside each chunk and loaded automatically at runtime; with `compile`, the bytecode is embedded in the standalone executable.
...
<Tabs>
  <Tab title="JavaScript">
    ...
    // ESM bytecode (generates .js + .js.jsc)
    await Bun.build({
      entrypoints: ["./index.tsx"],
      outdir: "./out",
      bytecode: true,
  ...
  <Tab title="CLI">
    ...
    # ESM bytecode
    bun build ./index.tsx --outdir ./out --bytecode --format=esm
    ```
